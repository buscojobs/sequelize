...
module.exports = (function() {
  var QueryGenerator = {
    ...
    createTableQuery: function(tableName, attributes, options) {
      ...
      var query   = "IF OBJECT_ID('<%= table %>', 'U') IS NULL CREATE TABLE <%= table %> (<%= attributes%>)<%= comment %>"
        , primaryKeys = []
        , foreignKeys = {}
        , attrStr = []
      ...
      var values = {
        ...
        comment: options.comment && Utils._.isString(options.comment) ? " /* " + options.comment + " */" : ""
      }
      ...
      if (!!options.uniqueKeys) {
        Utils._.each(options.uniqueKeys, function(columns) {
          values.attributes += ", CONSTRAINT uniq_" + tableName + '_' + columns.fields.join('_') + " UNIQUE(" + Utils._.map(columns.fields, self.quoteIdentifier).join(", ") + ")";
        })
      }
    ...
    */
    dropTableQuery: function(tableName, options) {
      ...
      if (typeof(tableName) == 'object')
        tname = tableName.TABLE_NAME;
      else
        tname = tableName;
      tname = this.quoteTable(tname);
    ...
    addColumnQuery: function(tableName, attributes) {
      ...
      for (var attrName in attributes) {
        ...
        attrString.push(Utils._.template('`<%= attrName %>` <%= definition %>')({
          attrName: attrName,
          definition: this.mssqlDataTypeMapping(tableName, attrName, definition)
        }))
      }
    ...
    */
    ...
    deleteQuery: function(tableName, where, options) {
      ...
      var limit = this.addTopLimit(options);
      
      return "DELETE" + limit + " FROM " + table + " WHERE " + where
    },
    ...
    addIndexQuery: function(tableName, attributes, options) {
      ...
      if(options.indicesType === 'FULLTEXT'){
        ...
        if (Utils.isHash(dataType)) {
          ...
          if (dataType.type.toString() === DataTypes.ENUM.toString()) {
            if (Array.isArray(dataType.values) && (dataType.values.length > 0)) {
              template = "VARCHAR NOT NULL CHECK (" + name + " IN(" + Utils._.map(dataType.values, function(value) {
    ...
    addTopLimit: function(options, query){
      query = query || ""
    ...
    addOrderLimitAndOffset: function(options, query){
      query = query || "";
      if(options.offset) {
    ...
    quoteIdentifier: function(identifier, force) {
      ...
      if(identifier.length == 0 || identifier[0] == "[") return identifier
      return "[" + identifier + "]";
    },
    /**
    ...
    getForeignKeysQuery: function(tableName, schemaName) {
      return "SELECT f.name AS constraint_name FROM sys.foreign_keys AS f INNER JOIN sys.foreign_key_columns AS fc ON f.OBJECT_ID = fc.constraint_object_id WHERE OBJECT_NAME (f.parent_object_id) = '" + tableName.TABLE_NAME + "'";
    },
    ...
    dropForeignKeyQuery: function(tableName, foreignKey) {
      return 'ALTER TABLE ' + this.quoteTable(tableName.TABLE_NAME) + ' DROP CONSTRAINT ' + this.quoteIdentifier(foreignKey) + ';'
    },
    ...
    */
    ...
    selectQuery: function(tableName, options, model) {
      ...
      var limitOrder = this.addTopLimit(options, query)
      ...
      if(options.order && options.offset) {
        if(mainQueryOrder.length) {
          mainQueryItems.push(this.addOrderLimitAndOffset(options, query));
        }
        if(subQueryOrder.length) {
          subQueryItems.push(this.addOrderLimitAndOffset(options, query));
        }
      } else if(options.offset) {
        mainQueryItems.push(" ORDER BY [id]")
        mainQueryItems.push(this.addOrderLimitAndOffset(options, query));
      }
    ...
    mssqlDataTypeMapping: function(tableName, attr, dataType) {
      ...
      if (Utils._.includes(dataType, 'TINYINT(1)')) {
        dataType = dataType.replace(/TINYINT\(1\)/, 'TINYINT')
      }
